...
1. Run the `preversion` script. These scripts have access to the old
...
1. Bump `version` in `package.json` as requested (`patch`, `minor`,
   `major`, etc).
1. Run the `version` script. These scripts have access to the new `version`
...
1. Commit and tag.
1. Run the `postversion` script. Use it to clean up the file system or
