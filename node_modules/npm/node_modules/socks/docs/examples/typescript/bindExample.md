...
1. Client \<-(port)- Proxy    (Tells the origin client which port it opened)
1. Client2 --> Proxy    (Other client connects to the proxy on this port)
1. Client \<--(client2's host info)  (Proxy tells the origin client who connected to it)
1. Original connection to the proxy is now a full TCP stream between client (you) and client2.
1. Client \<--> Proxy \<--> Client2
