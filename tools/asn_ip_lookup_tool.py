...
class ASNIPInput(BaseModel):
    ...
    ip_address: str = Field(..., description="IP address to look up ASN and network information for")
...
class ASNIPLookupTool(BaseTool):
    ...
    description: str = "Looks up the ASN (Autonomous System Number), associated IP network blocks (CIDRs), and organization name for a given IP address using WHOIS data."
    ...
    def _run(self, ip_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Found ASN {asn_info['asn']} ({asn_info['organization_name']}) for IP {ip_address}")
            return asn_info

        except Exception as e:
            logger.error(f"Error during ASN/IP lookup for {ip_address}: {e}", exc_info=True)
