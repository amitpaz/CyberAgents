...
class SemgrepInput(BaseModel):
    ...
    repo_url: Optional[str] = Field(None, description="URL of the Git repository to scan")
    ...
    language: Optional[str] = Field(
        None,
        description="Optional specific language for Semgrep (--lang flag). Recommended for code_snippet.",
    )
    ...
    download_folder: Optional[str] = Field(
        None,
        description="If repo_url was provided, optional path to clone into. Defaults to a temporary directory.",
    )
    ...
    def check_input_source(cls, values):
        ...
        if len(provided_sources) != 1:
            raise ValueError(
                f"Exactly one of {', '.join(sources)} must be provided. Found: {len(provided_sources)}"
            )
        return values
...
class SemgrepTool(BaseTool):
    ...
    description: str = (
        ...
        "using `semgrep scan --config=auto`."
    )
    ...
    def _find_semgrep_executable(self):
        ...
        if not SemgrepTool._checked_semgrep:
            ...
            if not SemgrepTool._semgrep_executable:
                logger.warning(
                    "Semgrep executable not found in PATH. The semgrep_scanner tool will not work."
                )
            else:
                logger.info(
    ...
    def _run(  # Synchronous execution for simplicity with subprocess
        ...
        try:
            ...
            if repo_url:
                if download_folder:
                    ...
                    try:
                        ...
                        clone_path = clone_path / f"semgrep_clone_{uuid.uuid4().hex[:8]}"
                        os.makedirs(clone_path)
                    except OSError as e:
                        return {"error": f"Failed to create or use download_folder {clone_path}: {e}"}
                ...
                if not os.path.exists(resolved_path):
                    return {"error": f"Provided local_path does not exist: {resolved_path}"}
                ...
                logger.info(f"Scanning code snippet in temporary file: {scan_target_path}")
            ...
            scan_results = self._run_semgrep_scan(scan_target_path, language)
            return scan_results # Return the raw results dict
            ...
            if temp_file_obj:
                try:
                    ...
                    logger.warning(f"Error closing temporary file {scan_target_path}: {e}")
            ...
            if cleanup_path and os.path.exists(cleanup_path):
                try:
                    ...
                    logger.error(f"Failed to cleanup temporary path {cleanup_path}: {e}")
    ...
    def _clone_repository(self, repo_url: str, clone_path: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=300, check=False # 5 min timeout
            )
    ...
    def _run_semgrep_scan(
        self, scan_path: str, language: Optional[str]
    ) -> Dict[str, Any]:
        """Runs `semgrep scan --config=auto` on the specified path."""
        cmd = [
            self._semgrep_executable,
            "scan",
            "--config=auto", # Use auto-detection for rules
            # "--json", # Optional: uncomment for JSON output, otherwise raw text
        ]
        if language:
        ...
        try:
            ...
            return {"error": "Semgrep scan timed out", "returncode": -1}
             ...
             # Should be caught by __init__ check, but handle defensively
            ...
            return {"error": f"An unexpected error occurred during scan: {e}", "returncode": -1}
