...
class PolicySyncManager:
    ...
    def __init__(self, repo_url: str = SEMGREP_REPO_URL, 
    ...
    def _find_policy_files(self, temp_dir: str, languages: List[str]) -> Dict[str, List[Path]]:
        """
        ...
        security_dirs = [
            ...
            "terraform", "docker", "kubernetes", "secrets", "security",
            "auth", "injection", "xss", "sqli", "csrf", "crypto",
            "command-injection", "path-traversal", "insecure-transport",
            "open-redirect", "xxe", "deserialization"
        ]
        
        # Priority patterns for security issues
        security_patterns = [
            r"injection", r"xss", r"sqli", r"command", r"traversal", 
            r"csrf", r"ssrf", r"access-control", r"auth", r"security",
            r"vulnerab", r"exploit", r"attack", r"auth", r"token",
            r"secret", r"password", r"cred", r"key", r"cert", 
            r"permission", r"sandbox", r"overflow", r"bound", r"race",
            r"deadlock", r"dos", r"backdoor", r"bypass", r"brute-force",
            r"privil", r"escalat", r"sign", r"crypto", r"encrypt", 
            r"decrypt", r"hash", r"random", r"trust", r"validate"
        ]
        ...
        for language in languages:
                ...
                if sec_path.exists() and sec_path.is_dir():
                    ...
                    for yaml_file in sec_path.glob("**/*.yml"):
                        try:
                            ...
                            if f"language: {language}" in content.lower() or f"languages: ['{language}'" in content.lower() or f'languages: ["{language}"' in content.lower():
            ...
            logger.info(f"Found {len(policy_files[language])} policy files for {language}")
        
        return policy_files
    
    def _copy_policies(self, policy_files: Dict[str, List[Path]], 
                      temp_dir: str) -> Dict[str, int]:
        """
        ...
        for language, files in policy_files.items():
            ...
            for i, file_path in enumerate(files):
                ...
                try:
                    ...
                    logger.error(f"Error copying {file_path} to {lang_dir / new_name}: {str(e)}")
