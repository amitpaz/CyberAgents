...
class ShodanHostInput(BaseModel):
    ...
    domain: str = Field(..., description="Domain name to search for associated hosts in Shodan")
...
class ShodanHostSearchTool(BaseTool):
    ...
    description: str = ("Searches Shodan for hosts, open ports, and services associated with a given domain. "
    ...
    def __init__(self, **kwargs):
        ...
        if self.api_key:
            ...
            logger.warning("SHODAN_API_KEY not found in environment variables. ShodanHostSearchTool will be unavailable.")
    ...
    def _run(self, domain: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Unexpected error during Shodan search for {domain}: {e}", exc_info=True)
