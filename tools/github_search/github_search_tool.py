...
class GitHubRateLimiter:
    ...
    def get_paginated_results(self, url, headers=None, params=None):
        ...
        while current_url:
            response, success = self.make_request(current_url, headers=headers, params=params)
...
class GitHubSearchTool(BaseTool):
    ...
    def _search_repository(self, repo: str) -> str:
        ...
        response, success = self.rate_limiter.retry_with_backoff(
            lambda: (response := requests.get(url, headers=self._get_headers()), response.status_code == 200)
        )
    ...
    def _analyze_repository(self, repo_data: Dict[str, Any]) -> str:
        ...
        medium_findings = self._search_for_patterns(repo_name, patterns["medium_priority"])
        if medium_findings:
        ...
        file_findings = self._search_for_patterns(repo_name, patterns["file_targets"], is_path=True)
        if file_findings:
        ...
        result += "* For API keys or credentials, revoke and rotate them if they are valid.\n"
    ...
    def _search_for_patterns(self, repo_name: str, patterns: List[str], is_path: bool = False) -> str:
        """
        ...
        response, success = self.rate_limiter.retry_with_backoff(
            lambda: (response := requests.get(url, headers=self._get_headers(), params=params), response.status_code == 200)
        )
                    ...
                    if filename.endswith(".yaml") or filename.endswith(".yml"):
                        ...
                        with open(file_path, "r") as f:
                            ...
                            for pattern_entry in policy_data.get("patterns", []):
                                severity = pattern_entry.get("severity", "MEDIUM").upper()
