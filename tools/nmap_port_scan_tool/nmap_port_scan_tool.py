...
def is_valid_nmap_target(target: str) -> bool:
    ...
    return is_valid_ip_address_or_network(target) or is_potentially_valid_hostname(target)
...
def sanitize_nmap_arguments(arguments: Optional[str]) -> str:
    ...
    if re.search(r'[;&|`$\(\)\{\}<>!\\]|\.\./', arguments):
        logger.warning(f"Potentially unsafe characters detected in Nmap arguments: '{arguments}'. Using default arguments.")
        return default_args
    # Allow common safe args (letters, numbers, hyphen, space, comma, dot)
    if not re.match(r'^[a-zA-Z0-9\s\-,.\\/]+$', arguments):
        logger.warning(f"Disallowed characters detected in Nmap arguments: '{arguments}'. Using default arguments.")
        return default_args
    ...
    for flag in dangerous_flags:
        if flag in arguments.split(): # Check as whole words
            logger.warning(f"Potentially dangerous Nmap flag '{flag}' detected in arguments: '{arguments}'. Using default arguments.")
            return default_args
...
class NmapPortScanTool(BaseTool):
    ...
    def _run(self, targets: str, ports: Optional[str] = None, arguments: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not is_valid_nmap_target(targets):
            ...
            return {"error": f"Invalid target format provided: '{targets}'. Please provide a valid IP, hostname, or CIDR network."}
        
        if not is_valid_nmap_ports(ports):
             logger.error(f"NmapPortScanTool received invalid ports input: '{ports}'")
             return {"error": f"Invalid ports format provided: '{ports}'. Use comma-separated numbers or ranges (e.g., '80,443', '1-1024')."}
