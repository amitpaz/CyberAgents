...
class EmailValidationTool(BaseTool):
    ...
    description: str = (
        "Validates SPF and DMARC DNS records for a given domain and suggests improvements."
    )
    ...
    def _run(self, domain: str) -> Dict[str, Any]:
        ...
        results = {
            "spf": {
                "record": None,
                "valid": False,
                "suggestion": "No SPF record found.",
            },
            "dmarc": {
                "record": None,
                "valid": False,
                "suggestion": "No DMARC record found.",
            },
        }
        ...
        try:
            ...
            for record in txt_records:
                ...
                if record_text.startswith("v=spf1"):
                    ...
                    results["spf"][
                        "suggestion"
                    ] = "SPF record found. Further validation may be needed for correctness (e.g., include mechanisms, qualifiers like -all)."
                    # More complex validation could be added here
                    if (
                        "-all" not in spf_record.lower()
                        and "~all" not in spf_record.lower()
                    ):
                        results["spf"][
                            "suggestion"
                        ] += " Consider adding a strong failure mechanism like '-all' or '~all'."
                    break  # Found the first SPF record
            if not spf_record:
                results["spf"][
                    "suggestion"
                ] = "No SPF record found. Suggest creating one, e.g., 'v=spf1 mx -all' or use an SPF generator."

        except dns.resolver.NoAnswer:
            results["spf"][
                "suggestion"
            ] = "No TXT records found for the domain, therefore no SPF record."
                    ...
                    if "p=" in record_text:
                        ...
                        if policy:
                            ...
                            if policy_value == "none":
                                results["dmarc"][
                                    "suggestion"
                                ] = "DMARC record found with policy 'none'. Consider changing to 'quarantine' or 'reject' for better protection after monitoring reports."
                            else:
                                results["dmarc"][
                                    "suggestion"
                                ] = f"DMARC record found with policy '{policy_value}'. This enforces email authentication."
                        else:
                            results["dmarc"][
                                "suggestion"
                            ] = "DMARC record found, but policy tag 'p=' seems malformed."
                            results["dmarc"][
                                "valid"
                            ] = False  # Invalid if policy is missing/malformed
                    else:
                        results["dmarc"][
                            "suggestion"
                        ] = "DMARC record found, but required policy tag 'p=' is missing."
                        results["dmarc"]["valid"] = False
                    break  # Found DMARC
            if not dmarc_record:
                results["dmarc"][
                    "suggestion"
                ] = "No DMARC record found at _dmarc.{domain}. Suggest creating one to enforce SPF/DKIM alignment and receive reports."

        except dns.resolver.NoAnswer:
            results["dmarc"][
                "suggestion"
            ] = f"No TXT records found for _dmarc.{domain}. No DMARC policy defined."
        except dns.resolver.NXDOMAIN:
            # This is expected if the domain exists but has no DMARC record
            results["dmarc"][
                "suggestion"
            ] = f"Subdomain _dmarc.{domain} does not exist. No DMARC policy defined."
        except Exception as e:
