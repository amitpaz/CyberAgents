...
def crew():
    ...
    try:
        ...
        pytest.fail(f"Failed to initialize DomainIntelligenceCrew for security tests: {e}")
...
def test_input_handling_sql_injection(crew):
    ...
    results = crew.run_analysis(prompt)
    assert "error" in results, f"Expected error for SQL injection pattern, got: {results}"
...
def test_input_handling_command_injection(crew):
    ...
    results = crew.run_analysis(prompt)
    assert "error" in results, f"Expected error for command injection pattern, got: {results}"
...
def test_input_handling_path_traversal(crew):
    ...
    results = crew.run_analysis(prompt)
    assert "error" in results, f"Expected error for path traversal pattern, got: {results}"
...
def test_input_handling_sensitive_data(crew):
    ...
    results = crew.run_analysis(prompt)
    assert "error" in results, f"Expected error for sensitive data pattern, got: {results}"
...
def test_input_handling_memory_exhaustion(crew):
    ...
    results = crew.run_analysis(prompt)
    assert "error" in results, f"Expected error for memory exhaustion pattern, got: {results}" 