...
def mock_env():
    ...
    with patch.dict(os.environ, {"SHODAN_API_KEY": "test_key", "OPENAI_API_KEY": "test_key"}, clear=True):
        yield

# Apply mocks needed for ExposureAnalystAgent initialization
@patch('tools.nmap_port_scan_tool.subprocess.run') # Mock nmap executable check
@patch('tools.nmap_port_scan_tool.nmap.PortScanner') # Mock nmap library scanner init
@patch('tools.shodan_tool.shodan.Shodan') # Mock Shodan client
def test_exposure_analyst_agent_initialization(
    ...
    mock_nmap_scanner.return_value = mock_nmap_scanner_instance # Ensure the constructor returns our mock
