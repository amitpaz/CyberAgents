...
def test_validate_schema():
    ...
    assert (
        validate_schema(invalid_data_missing, schema) is False
    ), "Missing required field passed schema check"
    assert (
        validate_schema(invalid_data_extra, schema) is False
    ), "Extra property passed schema check"
...
def test_validate_agent_yaml_valid(tmp_path):
    ...
    assert (
        validate_agent_yaml(str(yaml_file)) is True
    ), "Valid agent YAML failed validation"
...
def test_validate_agent_yaml_invalid(tmp_path):
    ...
    assert (
        validate_agent_yaml(str(yaml_file_missing)) is False
    ), "Invalid agent YAML (missing field) passed validation"
    assert (
        validate_agent_yaml(str(yaml_file_extra)) is False
    ), "Invalid agent YAML (extra field) passed validation"
