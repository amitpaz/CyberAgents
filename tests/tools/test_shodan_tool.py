...
from tools import ShodanHostSearchTool
from tools.shodan_search.shodan_tool import ShodanHostInput
...
@patch("tools.shodan_search.shodan_tool.shodan.Shodan")  # Mock the client class
...
@patch("tools.shodan_search.shodan_tool.shodan.Shodan")
def test_run_valid_domain(mock_shodan_client):
    ...
    mock_shodan_instance.search.return_value = {
        "total": 1,
        "matches": [
            {"ip_str": "1.2.3.4", "port": 80, "hostnames": ["test.example.com"]}
        ],
    }
...
@patch.dict(os.environ, {"SHODAN_API_KEY": "fake_shodan_key"}, clear=True)
@patch("tools.shodan_search.shodan_tool.shodan.Shodan")  # Still need mock for init
