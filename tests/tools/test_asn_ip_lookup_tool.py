...
from tools import ASNIPLookupTool
...
@patch("tools.asn_ip_lookup_tool.asn_ip_lookup_tool.IPWhois")  # Still need mock even if not called
def test_run_invalid_ip_inputs(mock_ipwhois, asn_tool, invalid_ip, description):
    ...
    print(
        f"Testing {description}: Input='{invalid_ip}', Result='{result}'"
    )  # Debugging output
    assert "error" in result
